...
def stitchImagesRandomized(alpha):
    ...
    while not result:
        ...
        img_ = cv2.imread(r"image_0001.jpg")
        img1 = cv2.cvtColor(img_, cv2.COLOR_BGR2GRAY)
        img = cv2.imread(r"image_0003.jpg")
        ...
        startX = int(random.randint(0,img2.shape[1] - rows))
        #startY = int(img2.shape[1] / 2 - 1)
        startY= 0
        img2Holder = img2
        #print(img2.shape)

        img2 = img2[0:-1,startX:rows+startX]
        #print(img2.shape)
        #cv2.imshow("test",img2)
        #cv2.waitKey(0)
        #first position is the y value, 2nd position is the x value
        #img2 = img2[0:-1, img2.shape[1] - 1000:-1]
       ...
       # cv2.imshow("window",img2)
        ...
        for kp in kp2:
            kp.pt = (kp.pt[0] + startX,kp.pt[1] + startY)
        ...
        match_time =  time.time() -startTime
